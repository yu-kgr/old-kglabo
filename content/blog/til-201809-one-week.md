...
先程の `192.168.2.1/32` を `192.168.0.0/16`だと、末尾の`8bit*2`の箇所、 `.0.0`が`*`的な扱いになる。
- つまり、192.168.0.0 〜 192.168.255.255 という感じ
...
- /24 : 256のIPアドレス帯（※実際は251）
  - ※AWSの場合は5つ分のアドレスが予約されている


---

IPアドレスは `10.20.30.40/32`のような形で記載され、8bitの
